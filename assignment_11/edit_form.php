...
<body>
	<ol class="breadcrumb">
		...
		<li class="breadcrumb-item"><a href="details.php?id=<?php echo $_GET['id']; ?>">Details</a></li>
	...
	<div class="container">
		...
		<form action="edit_confirmation.php" method="POST">
					...
					<select name="genre" id="genre-id" class="form-control">
						...
						<?php while( $row = $genres->fetch_assoc() ) : ?>
							...
							<?php if ( $row_dvd['genre_id'] == $row['genre_id'] ) : ?>
								...
								<option value="<?php echo $row['genre_id'] ?>" selected > <?php echo $row['genre'] ?> </option>
					...
					<select name="rating" id="rating-id" class="form-control">
						...
						<?php while( $row = $ratings->fetch_assoc() ) : ?>
							...
							<?php if ( $row_dvd['rating_id'] == $row['rating_id'] ) : ?>
								...
								<option value="<?php echo $row['rating_id'] ?>" selected> <?php echo $row['rating'] ?> </option>
					...
					<select name="label" id="label-id" class="form-control">
						...
						<?php while( $row = $labels->fetch_assoc() ) : ?>
							...
							<?php if ( $row_dvd['label_id'] == $row['label_id'] ) : ?>
								...
								<option value="<?php echo $row['label_id'] ?>" selected> <?php echo $row['label'] ?> </option>
					...
					<select name="format" id="format-id" class="form-control">
						...
						<?php while ( $row = $formats->fetch_assoc() ) : ?>
							...
							<?php if ( $row_dvd['format_id'] == $row['format_id'] ) : ?>
								...
								<option value="<?php echo $row['format_id'] ?>" selected> <?php echo $row['format'] ?> </option>
					...
					<select name="sound" id="sound-id" class="form-control">
						...
						<?php while ( $row = $sounds->fetch_assoc() ) : ?>
							...
							<?php if ( $row_dvd['sound_id'] == $row['sound_id'] ) : ?>
								...
								<option value="<?php echo $row['sound_id'] ?>" selected> <?php echo $row['sound'] ?> </option>
			...
			<div class="form-group row">
				...
				<div class="col-sm-9">
					<input type="date" class="form-control" id="date-id" name="date" value ="<?php echo $row_dvd['release_date'] ?>">
				</div>
			</div> <!-- .form-group -->
			...
			<input type="hidden" name="dvd_id" value="<?php echo $_GET['id']; ?>">
